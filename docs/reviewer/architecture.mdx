...
Page token содержит version, random `jti`, `iat`, embedded `exp` и visitor
`binding`. Payload подписан HMAC-SHA256 через `PAGE_TOKEN_SECRET`, а binding
вычислен domain-separated HMAC через `HASH_SECRET` для visitor ID и `jti`.
Backend проверяет signature, future skew, expiry и binding к текущей signed cookie.
Server dwell рассчитывается только после полной проверки token и не доверяет
client timestamps.
