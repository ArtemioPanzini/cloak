...
2. Сервер выпускает HMAC-signed page token `{ v, jti, iat, exp, binding }`,
   привязанный к текущей signed visitor cookie.
3. `telemetry.js` собирает агрегаты до submit.
4. JSON payload проходит type/range normalization.
5. Backend проверяет signature, embedded expiry и visitor binding; server dwell
   строится только для полностью valid token.
...
Direct submit без token или с token из другой visitor session также получает
`WHITEPAGE`: non-valid token не проходит `OFFER` eligibility.
